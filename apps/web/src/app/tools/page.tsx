  ...
  return (
    <div className="animate-fade-in px-5 md:px-10 lg:px-12 pt-[58px] lg:pt-18 pb-18 flex flex-col gap-12 w-full max-w-[1000px] xl:max-w-[1600px]">
      <div className="flex flex-col gap-3">
        <div className="page-title">My tech stack</div>
        <div className="h-[56.6016px] flex flex-col justify-start">
          <div className="page-subtitle">
            Technologies I genuinely enjoy working with and confidently
            recommend.
          </div>
        </div>
      </div>
...
function Tool({ tool }: { tool: Tool }) {
  return (
    <a
      href={tool.link}
      target="_blank"
      className="flex gap-4 w-full border border-[#232347] bg-[#1a1a2e] rounded-xl p-4 items-center transition-colors duration-300 hover:bg-[#232347] cursor-default group"
    >
      <img
        src={tool.image}
        alt={tool.name}
      ...
      <ExternalLink className="w-5 h-5 opacity-50 group-hover:opacity-100 transition-all duration-300 group-hover:rotate-45 flex-shrink-0" />
    </a>
  );
}
